auto SonolusLevelsResultUpload = [](client_conn conn, http_request request, param argv){
    ...
    string key = request.argv["sonolus-upload-key"];
    vector<Record<LevelItem> > records = recordsList<LevelItem>("name = \"" + key + "\"", "");
    if (records.size() == 0) quickSendMsg(404, "Record not found.");
    auto record = records[0];
    string configuration = files[record.replay.configuration.hash];
    data = files[record.replay.data.hash];
