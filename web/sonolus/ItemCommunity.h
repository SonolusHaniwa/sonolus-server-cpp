auto SonolusCommunityInfo = [](client_conn conn, http_request request, param argv) {
    ...
    bool isLike = isLogin && (db.query("SELECT uid FROM Like WHERE targetType=\"" + string(argv[0]) + "\" AND targetName=\"" + string(argv[1]) + "\" AND uid=\"" + getUserProfile(request).id + "\"").size());
    ...
    if (appConfig[string(argv[0]) + ".enableLike"].asBool()) {
        int likes = db.query("SELECT uid FROM Like WHERE targetType=\"" + string(argv[0]) + "\" AND targetName=\"" + string(argv[1]) + "\"").size();
...
auto SonolusCommunitySubmit = [](client_conn conn, http_request request, param argv) {
    ...
    bool isLike = db.query("SELECT uid FROM Like WHERE targetType=\"" + argv[0] + "\" AND targetName=\"" + argv[1] + "\" AND uid=\"" + profile.id + "\"").size();
    ...
    if ($_POST["type"] == "like" && appConfig[argv[0] + ".enableLike"].asBool() && !isLike)
        db.execute("INSERT INTO Like (targetType, targetName, uid) VALUES (\"" + argv[0] + "\", \"" + argv[1] + "\", \"" + profile.id + "\")");
    if ($_POST["type"] == "dislike" && appConfig[argv[0] + ".enableLike"].asBool() && isLike)
        db.execute("DELETE FROM Like WHERE targetType=\"" + argv[0] + "\" AND targetName=\"" + argv[1] + "\" AND uid=\"" + profile.id + "\"");
