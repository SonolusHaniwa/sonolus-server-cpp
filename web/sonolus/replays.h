...
auto sonolus_replays = [](client_conn conn, http_request request, param argv){
    ...
    Section<ReplayItem> section = replaysList("name = \"" + str_replace("\"", "\\\"", argv[0]) + "\" AND localization = \"" + $_GET["localization"] + "\"", 1, 1);
    if (section.items.size() == 0) section = replaysList("name = \"" + str_replace("\"", "\\\"", argv[0]) + "\"", 1, 1);
    ...
    Section<ReplayItem> recommended = replaysList("author = \"" + str_replace("\"", "\\\"", item.author) + "\" AND (localization = \"" + $_GET["localization"] + "\" OR localization = \"default\")", 1, 10);
