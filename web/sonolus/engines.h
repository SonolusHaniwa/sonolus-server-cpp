...
auto sonolus_engines = [](client_conn conn, http_request request, param argv){
    ...
    Section<EngineItem> section = engineList("name = \"" + str_replace("\"", "\\\"", argv[0]) + "\" AND localization = \"" + $_GET["localization"] + "\"", 1, 1);
    if (section.items.size() == 0) section = engineList("name = \"" + str_replace("\"", "\\\"", argv[0]) + "\"", 1, 1);
    ...
    Section<EngineItem> recommended = engineList("author = \"" + str_replace("\"", "\\\"", item.author) + "\" AND (localization = \"" + $_GET["localization"] + "\" OR localization = \"default\")", 1, 10);
