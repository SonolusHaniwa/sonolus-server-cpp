...
auto web_engines_list = [](client_conn conn, http_request request, param argv) {
    ...
    argList["html.navbar"] = fetchNavBar(argList["language.engines"]).output();
    ...
    argList["html.enginesBottom"] = fetchBottomBar(sonolusUrl, topUrl, previousUrl, nextUrl, bottomUrl, page, section.pageCount).output();
