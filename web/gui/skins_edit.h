...
auto web_skins_edit = [](client_conn conn, http_request request, param argv){
    ...
    string body = readFile("./web/html/pages/skins_edit.html");
    ...
    argList["html.navbar"] = fetchNavBar(argList["language.skinCreate"], checkLogin(request)).output();
    argList["html.skinsCreateOption"] = "";
    auto dataSet2 = (new DB_Controller)->query("SELECT * FROM Skin WHERE name = \"" + argv[0] +
    ...
    for (int i = 0; i < SkinCreate.options.size(); i++) {
        ...
        if (v.type == "text") argList["html.skinsCreateOption"] += fetchSearchText(v.text.query, v.text.name, v.text.placeholder, dataSet[v.text.query], i != 0).output();
        if (v.type == "toggle") argList["html.skinsCreateOption"] += fetchSearchToggle(v.toggle.query, v.toggle.name, atoi(dataSet[v.toggle.query].c_str()), i != 0).output();
        if (v.type == "select") {
            ...
            argList["html.skinsCreateOption"] += fetchSearchSelect(v.select.query, v.select.name, v.select.values, def, i != 0).output();
        } if (v.type == "slider") argList["html.skinsCreateOption"] += fetchSearchSlider(v.slider.query, v.slider.name, atoi(dataSet[v.slider.query].c_str()), v.slider.min, v.slider.max, v.slider.step, i != 0).output();
        if (v.type == "file") argList["html.skinsCreateOption"] += fetchSearchFile(v.file.query, v.file.name, dataSet[v.file.query], i != 0).output();
    }
