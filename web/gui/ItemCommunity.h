void quickGUICommunity(http_request request, string type, string name, argvar& argList, string& detailsIcon) {
    ...
    if (appConfig[type + ".enableLike"].asBool()) {
        ...
        int likes = db.query("SELECT uid FROM LikeTable WHERE targetType = \"" + type + "\" AND targetName = \"" + name + "\"").size();
    ...
    if (appConfig[type + ".enableRating"].asBool()) {
        ...
        int ratings = db.query("SELECT uid FROM Rating WHERE targetType = \"" + type + "\" AND targetName = \"" + name + "\"").size();
        double rating = atof(db.query("SELECT AVG(rating) FROM Rating WHERE targetType = \"" + type + "\" AND targetName = \"" + name + "\"")[0]["AVG(rating)"].c_str());
        argList["html.community.text"] += ItemCommunityComment("", "System", 0, "Rating: " + to_string(rating) + "(" + to_string(ratings) + ")", {}).toHTMLObject().output();
        auto myRating = db.query("SELECT rating FROM Rating WHERE targetType = \"" + type + "\" AND targetName = \"" + name + "\" AND uid = \"" + getUserProfile(request).id + "\"");
