...
auto web_engines_edit = [](client_conn conn, http_request request, param argv){
    ...
    string body = readFile("./web/html/pages/engines_edit.html");
    ...
    argList["page.title"] = argList["language.engineEdit"] + " | " + appConfig["server.title"].asString();
    argList["html.navbar"] = fetchNavBar(argList["language.engineEdit"], checkLogin(request)).output();
    ...
    for (int i = 0; i < EngineCreate.options.size(); i++) {
        ...
        if (v.type == "text") argList["html.enginesCreateOption"] += fetchSearchText(v.text.query, v.text.name, v.text.placeholder, dataSet[v.text.query], i != 0).output();
        if (v.type == "toggle") argList["html.enginesCreateOption"] += fetchSearchToggle(v.toggle.query, v.toggle.name, atoi(dataSet[v.toggle.query].c_str()), i != 0).output();
        if (v.type == "select") {
            ...
            argList["html.enginesCreateOption"] += fetchSearchSelect(v.select.query, v.select.name, v.select.values, def, i != 0).output();
        } if (v.type == "slider") argList["html.enginesCreateOption"] += fetchSearchSlider(v.slider.query, v.slider.name, atoi(dataSet[v.slider.query].c_str()), v.slider.min, v.slider.max, v.slider.step, i != 0).output();
        if (v.type == "file") argList["html.enginesCreateOption"] += fetchSearchFile(v.file.query, v.file.name, dataSet[v.file.query], i != 0).output();
    }
