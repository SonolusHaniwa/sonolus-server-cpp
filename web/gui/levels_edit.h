...
auto web_levels_edit = [](client_conn conn, http_request request, param argv){
    ...
    string body = readFile("./web/html/pages/levels_edit.html");
    ...
    argList["page.title"] = argList["language.levelEdit"] + " | " + appConfig["server.title"].asString();
    argList["html.navbar"] = fetchNavBar(argList["language.levelEdit"], checkLogin(request)).output();
    argList["html.levelsCreateOption"] = "";
    auto dataSet2 = (new DB_Controller)->query("SELECT * FROM Level WHERE name = \"" + argv[0] +
    ...
    for (int i = 0; i < LevelCreate.options.size(); i++) {
        ...
        if (v.type == "text") argList["html.levelsCreateOption"] += fetchSearchText(v.text.query, v.text.name, v.text.placeholder, dataSet[v.text.query], i != 0).output();
        if (v.type == "toggle") argList["html.levelsCreateOption"] += fetchSearchToggle(v.toggle.query, v.toggle.name, atoi(dataSet[v.toggle.query].c_str()), i != 0).output();
        if (v.type == "select") {
            ...
            argList["html.levelsCreateOption"] += fetchSearchSelect(v.select.query, v.select.name, v.select.values, def, i != 0).output();
        } if (v.type == "slider") argList["html.levelsCreateOption"] += fetchSearchSlider(v.slider.query, v.slider.name, atoi(dataSet[v.slider.query].c_str()), v.slider.min, v.slider.max, v.slider.step, i != 0).output();
        if (v.type == "file") argList["html.levelsCreateOption"] += fetchSearchFile(v.file.query, v.file.name, dataSet[v.file.query], i != 0).output();
    }
