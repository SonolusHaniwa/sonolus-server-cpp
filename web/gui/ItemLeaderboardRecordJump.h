auto GUILeaderboardRecordJump = [](client_conn conn, http_request request, param argv) {
    if (argv[0] != "levels" && 
        argv[0] != "skins" && 
        argv[0] != "backgrounds" && 
        argv[0] != "effects" && 
    ...
    argList["pages.current"] = to_string(atoi(argv[3].c_str()) + 1);
    argList["url.base"] = "/" + argv[0] + "/" + argv[1] + "/leaderboard/" + argv[2] + "/list?";
