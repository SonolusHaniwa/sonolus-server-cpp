...
auto web_backgrounds = [](client_conn conn, http_request request, param argv) {
    ...
    Section<BackgroundItem> background = backgroundList("name = \"" + str_replace("\"", "\\\"", argv[0]) + "\" AND localization = \"" + cookie["lang"] + "\"", 1, 1);
    if (background.items.size() == 0) background = backgroundList("name = \"" + str_replace("\"", "\\\"", argv[0]) + "\"", 1, 1);
    ...
    Section<BackgroundItem> recommended = backgroundList("author = \"" + str_replace("\"", "\\\"", item.author) + "\" AND (localization = \"" + cookie["lang"] + "\" OR localization = \"default\")", 1, 5);
