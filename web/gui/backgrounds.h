...
auto web_backgrounds = [](client_conn conn, http_request request, param argv) {
    ...
    Section<BackgroundItem> background = backgroundList("name = \"" + str_replace("\"", "\\\"", argv[0]) + "\"", 1, 1);
    ...
    Section<BackgroundItem> recommended = backgroundList("author = \"" + str_replace("\"", "\\\"", item.author) + "\"", 1, 5);
