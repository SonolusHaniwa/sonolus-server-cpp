...
auto web_skins = [](client_conn conn, http_request request, param argv) {
    ...
    Section<SkinItem> skin = skinsList("name = \"" + str_replace("\"", "\\\"", argv[0]) + "\" AND localization = \"" + cookie["lang"] + "\"", 1, 1);
    if (skin.items.size() == 0) skin = skinsList("name = \"" + str_replace("\"", "\\\"", argv[0]) + "\"", 1, 1);
    ...
    Section<SkinItem> recommended = skinsList("author = \"" + str_replace("\"", "\\\"", item.author) + "\" AND (localization = \"" + cookie["lang"] + "\" OR localization = \"default\")", 1, 5);
