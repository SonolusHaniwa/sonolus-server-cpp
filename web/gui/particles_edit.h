...
auto web_particles_edit = [](client_conn conn, http_request request, param argv){
    ...
    string body = readFile("./web/html/pages/particles_edit.html");
    ...
    argList["page.title"] = argList["language.particleEdit"] + " | " + appConfig["server.title"].asString();
    argList["html.navbar"] = fetchNavBar(argList["language.particleEdit"], checkLogin(request)).output();
    argList["html.particlesCreateOption"] = "";
    auto dataSet2 = (new DB_Controller)->query("SELECT * FROM Particle WHERE name = \"" + argv[0] +
    ...
    for (int i = 0; i < ParticleCreate.options.size(); i++) {
        ...
        if (v.type == "text") argList["html.particlesCreateOption"] += fetchSearchText(v.text.query, v.text.name, v.text.placeholder, dataSet[v.text.query], i != 0).output();
        if (v.type == "toggle") argList["html.particlesCreateOption"] += fetchSearchToggle(v.toggle.query, v.toggle.name, atoi(dataSet[v.toggle.query].c_str()), i != 0).output();
        if (v.type == "select") {
            ...
            argList["html.particlesCreateOption"] += fetchSearchSelect(v.select.query, v.select.name, v.select.values, def, i != 0).output();
        } if (v.type == "slider") argList["html.particlesCreateOption"] += fetchSearchSlider(v.slider.query, v.slider.name, atoi(dataSet[v.slider.query].c_str()), v.slider.min, v.slider.max, v.slider.step, i != 0).output();
        if (v.type == "file") argList["html.particlesCreateOption"] += fetchSearchFile(v.file.query, v.file.name, dataSet[v.file.query], i != 0).output();
    }
