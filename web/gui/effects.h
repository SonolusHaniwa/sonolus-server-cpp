...
auto web_effects = [](client_conn conn, http_request request, param argv) {
    ...
    Section<EffectItem> effect = effectsList("name = \"" + str_replace("\"", "\\\"", argv[0]) + "\" AND localization = \"" + cookie["lang"] + "\"", 1, 1);
    if (effect.items.size() == 0) effect = effectsList("name = \"" + str_replace("\"", "\\\"", argv[0]) + "\"", 1, 1);
    ...
    Section<EffectItem> recommended = effectsList("author = \"" + str_replace("\"", "\\\"", item.author) + "\" AND (localization = \"" + cookie["lang"] + "\" OR localization = \"default\")", 1, 5);
