...
auto web_replays_edit = [](client_conn conn, http_request request, param argv){
    ...
    string body = readFile("./web/html/pages/replays_edit.html");
    ...
    for (int i = 0; i < ReplayCreate.options.size(); i++) {
        ...
        if (v.type == "text") argList["html.replaysCreateOption"] += fetchSearchText(v.text.query, v.text.name, v.text.placeholder, dataSet[v.text.query], i != 0).output();
        if (v.type == "toggle") argList["html.replaysCreateOption"] += fetchSearchToggle(v.toggle.query, v.toggle.name, atoi(dataSet[v.toggle.query].c_str()), i != 0).output();
        if (v.type == "select") {
            ...
            argList["html.replaysCreateOption"] += fetchSearchSelect(v.select.query, v.select.name, v.select.values, def, i != 0).output();
        } if (v.type == "slider") argList["html.replaysCreateOption"] += fetchSearchSlider(v.slider.query, v.slider.name, atoi(dataSet[v.slider.query].c_str()), v.slider.min, v.slider.max, v.slider.step, i != 0).output();
        if (v.type == "file") argList["html.replaysCreateOption"] += fetchSearchFile(v.file.query, v.file.name, dataSet[v.file.query], i != 0).output();
    }
