...
vector<PostItem> postsList(string filter, string order, int st = 1, int en = 20) {
    ...
    for (int i = 0; i < res.size(); i++) {
        ...
        PostItem data = PostItem(
            ...
            str_replace("\\n", "\n", res[i]["description"]),
            (appConfig["server.enableSSL"].asBool() ? "https://" : "http://") + appConfig["server.rootUrl"].asString() + "/sonolus/posts/" + res[i]["name"]
        ); list.push_back(data);
    } return list;
}
...
int postsCreate(PostItem item, string localization = "default") {
    ...
    auto res = db.query("SELECT id FROM Post WHERE name = \"" + item.name + "\" AND localization = \"" + localization + "\"");
    if (res.size() != 0) {
